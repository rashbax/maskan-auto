      ...
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white/95 backdrop-blur border-t border-line px-4 pt-3"
        ...
        <div className="max-w-6xl mx-auto flex items-center gap-3">
          <div className="shrink-0">
            <div className="font-bold text-[18px] tnum leading-none">${apt.price}<span className="text-[13px] text-inksoft font-semibold"> / {STR[lang].night1}</span></div>
            {nights > 0 && <div className="text-[12px] text-green-700 font-semibold mt-1">${apt.price * nights} · {STR[lang].night_n(nights)}</div>}
          </div>
          <div className="ml-auto">
            <Button size="lg" icon="bolt" onClick={onReserve}>
              <span className="whitespace-nowrap">{nights ? `${STR[lang].confirm_book} · $${apt.price * nights}` : STR[lang].select_dates}</span>
            </Button>
          </div>
        </div>
      </div>
